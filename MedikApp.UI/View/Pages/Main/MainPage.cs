...
namespace MedikApp.UI
{
    public partial class MainPage : UserControl
    {
        ...
        private async void MainPage_Load(object sender, EventArgs e)
        {
            ...
            person = await GetApiDataAsync(LogIn.UserInfo.tc);
            textBox2.Visible = false;
            textBox3.Visible = false;
            textBox1.Visible = false;
            button3.Visible = false;
            button4.Visible = false;
            Yas.Text = Helper.YasHesapla(person.DogumTarihi).ToString();
            label5.Text = person.Kilo;
            label7.Text = Helper.GetKanGurubuById(Int32.Parse(person.KanGurubu));
            label3.Text = person.Boy;
            label10.Text = person.Isim;
        }
        ...
        private async Task<Kullanici> GetApiDataAsync(string tc)
        {
        ...
        private async void button3_Click(object sender, EventArgs e)
        {
            ...
            int kanId = Helper.GetKanGurubuId(textBox2.Text);
            MessageBox.Show(telno + kanId.ToString());
            string tc = LogIn.UserInfo.tc;
            ...
            if (result == DialogResult.Yes)
            {
                ...
                {
                    if ((textBox1.Text == person.Boy) && (textBox2.Text == Helper.GetKanGurubuById(kanId)) && (textBox3.Text == person.Kilo))
                    {
